...
**discover signalhood or other stable interaction regularities, establish grounded semantic invariants, create compositional communication, preserve interactional/pragmatic distinctions where demonstrated, and identify both translatable and genuinely non-equivalent concepts?**
...
18. **Pragmatic function is also a hypothesis.** Addressee, audience, communicative function, background assumptions, repair, convention state, and sender reliability must be tested rather than inferred from human-looking form.
19. **Preserve tested invariants, not slogans.** Surface identity is neither necessary nor sufficient for faithful translation, but functional similarity is not sufficient either; form must be conserved when form itself carries a demonstrated distinction.

## R0.5 — pre-R1 information and experiment-integrity audit

Before the R1 substrates are compared, UNVTRSLR inserts a deliberately thin **R0.5** layer.
...
R2 also separates **operational relation verification** from **semantic interpretation confirmation**. A post-hoc label cannot be confirmed using the same evidence that selected it; fresh IID evidence is not enough if it preserves the original confound; and a stronger semantic claim must carry an explicit `SEMANTIC_SURPLUS_OBLIGATION`—some additional falsifiable prediction or qualification burden beyond ordinary operational success.
...
R3 retains the existing human-language and synthetic-language control phase and adds a scoped pragmatics/interaction qualification profile. It asks whether a grounded bridge still preserves meaning when reference alone is insufficient—for example when the same referent is used with a different communicative function, when perspective or addressee changes, when a convention drifts, when a sender is deceptive, when silence rather than an emitted symbol matters, or when modalities conflict.

R3 is downstream of R0.5 and R2. A pragmatic label cannot outrun the R0.5 claim ceiling or bypass R2 semantic-claim controls. If the target distinction was not empirically identifiable, was destroyed by preprocessing, was supplied by a hidden bridge, or was selected post-hoc without claim-discriminating confirmation, R3 cannot repair that defect by obtaining high interactional performance.
...
A future passing result may receive `PRAGMATICALLY_GROUNDED_WITHIN_TESTED_SCOPE`. There is intentionally no `UNIVERSAL_PRAGMATICS` status, and R3 cannot compensate for R0.5 or R2 failure.
...
21. pragmatic-function contrast under fixed denotation;
22. addressee/audience and perspective shifts;
23. targeted repair versus reflex repair;
24. pragmatic conservation under ambiguity, deception, and multimodal conflict.
...
`R0_5_R1_R2_DESIGN_BASELINED / R3_PRAGMATICS_DESIGN_SPECIFIED / IMPLEMENTATION_NOT_STARTED / NO_SEMANTIC_OR_PRAGMATIC_QUALIFICATION`

The repository currently defines a research program, a pre-R1 experiment-integrity audit, competing semantic representations, adversarial semantic-claim controls, and an R3 pragmatic-robustness design. It is not a proven universal language, universal ontology, universal pragmatics, or extraterrestrial communication solution. The strongest claims here are intentionally written so they can fail.
